...
import { CATEGORY_FILES } from "../src/constants/categoryFiles";
...
const Categories: React.FC = () => {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError("");
    fetch(`/Pedhe-Wale/pedhe_json/${file}`)
      .then((res) => {
      ...
      .then((data) => {
        console.log("Fetched JSON:", data); // ✅ Debug
      ...
      <main className="flex-grow-1  p-5 overflow-auto ">
        ...
        {loading ? (
          ...
          <div className="alert alert-danger p-3">
            ...
            <code>public/pedhe_json/{CATEGORY_FILES[selectedCategory]}</code>{" "}
            मौजूद है।
          </div>
        ) : filtered.length === 0 ? (
